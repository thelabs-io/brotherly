"""Request management for brotherly."""

from __future__ import annotations

import shutil
from datetime import datetime
from pathlib import Path

from brotherly.config import Config
from brotherly.models import QueuedTask, TaskStatus
from brotherly.script_parser import parse_script_header


class RequestManager:
    def __init__(self, config: Config) -> None:
        self.config = config
        self.config.ensure_dirs()

    def add_task(
        self,
        script_path: Path,
        requires_sudo: bool = False,
    ) -> QueuedTask:
        header = parse_script_header(script_path)
        task_id = QueuedTask.generate_id(header.title)
        script_filename = f"{task_id}.sh"

        dest = self.config.requests_dir / script_filename
        shutil.copy2(script_path, dest)
        dest.chmod(0o755)

        task = QueuedTask(
            id=task_id,
            script_filename=script_filename,
            title=header.title,
            description=header.description,
            queued_at=datetime.now().isoformat(),
            requires_sudo=requires_sudo,
        )

        meta_path = self.config.requests_dir / f"{task_id}.json"
        meta_path.write_text(task.to_json())

        return task

    def list_pending(self) -> list[QueuedTask]:
        tasks = []
        for json_file in sorted(self.config.requests_dir.glob("*.json")):
            task = QueuedTask.from_file(json_file)
            if task.status == TaskStatus.PENDING:
                tasks.append(task)
        return tasks

    def list_all(self) -> list[QueuedTask]:
        tasks = []
        for json_file in sorted(self.config.requests_dir.glob("*.json")):
            tasks.append(QueuedTask.from_file(json_file))
        return tasks

    def get_task(self, task_id: str) -> QueuedTask | None:
        meta_path = self.config.requests_dir / f"{task_id}.json"
        if not meta_path.exists():
            return None
        return QueuedTask.from_file(meta_path)

    def get_script_content(self, task: QueuedTask) -> str:
        script_path = self.config.requests_dir / task.script_filename
        return script_path.read_text()

    def get_script_path(self, task: QueuedTask) -> Path:
        return self.config.requests_dir / task.script_filename

    def update_task(self, task: QueuedTask) -> None:
        meta_path = self.config.requests_dir / f"{task.id}.json"
        meta_path.write_text(task.to_json())

    def log_path(self, task: QueuedTask) -> Path:
        return self.config.logs_dir / f"{task.id}.log"
